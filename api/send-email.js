...
module.exports = async (req, res) => {
  ...
  const {
    to, template, type, name, baseUrl,
    workerName, clientName, service, price, bookingId,
    specialty, description, clientEmail,
    // Quotation fields
  ...
  const emailData = buildEmail(tpl, {
  ...
  if (!emailData) {
    return res.status(400).json({ error: `Unknown template: ${tpl}` });
  }

  // Determine recipients: admin templates go to admins too
  let recipients = [to];
  if (tpl.startsWith('admin_') || tpl === 'new_quotation' || tpl === 'quotation_accepted' || tpl === 'quotation_rejected'
      || tpl === 'specialty_request' || tpl === 'new_specialty') {
    recipients = [...new Set([to, ...ADMINS])];
  }

  try {
    // Use Resend API
    const apiKey = process.env.RESEND_API_KEY || process.env.VOY_RESEND_KEY;
    if (!apiKey) {
...
function buildEmail(template, data) {
  const { siteUrl, to, name, workerName, clientName, service, price, bookingId,
          specialty, description, clientEmail,
  ...
  const templates = {
    ...
    booking_confirmed: {
      ...
      html: wrapEmail('Reserva Confirmada', '#059669', `
        <h2 style="color:#059669;margin:0 0 8px;">¡Reserva confirmada!</h2>
        <p style="color:#64748b;font-size:15px;">Hola <strong>${name || ''}</strong>, tu servicio ha sido agendado exitosamente. Tu especialista ya fue notificado y confirmó tu solicitud.</p>
        <a href="${siteUrl}/client/" style="display:inline-block;padding:12px 28px;background:#059669;color:white;text-decoration:none;border-radius:8px;font-weight:700;margin-top:16px;">Ver mis servicios</a>
      `),
    },
    ...
    new_request: {
      ...
      html: wrapEmail('Nueva Solicitud', '#0EA5E9', `
        <h2 style="color:#0EA5E9;margin:0 0 8px;">Nueva solicitud</h2>
        <p style="color:#64748b;font-size:15px;">Hola <strong>${name || ''}</strong>, un cliente cercano necesita tus servicios. Revisa y responde desde tu panel de especialista.</p>
        <a href="${siteUrl}/worker/" style="display:inline-block;padding:12px 28px;background:#0EA5E9;color:white;text-decoration:none;border-radius:8px;font-weight:700;margin-top:16px;">Ver solicitudes</a>
      `),
    },
    ...
    specialty_request: {
      ...
      html: wrapEmail('Solicitud de Especialidad', '#D97706', `
        <h2 style="color:#D97706;margin:0 0 8px;">Nueva solicitud de especialidad</h2>
        <p style="color:#64748b;font-size:15px;">Un cliente busca un especialista que no existe en la plataforma.</p>
        <table style="width:100%;border-collapse:collapse;margin:16px 0;">
          <tr><td style="padding:8px;color:#64748b;">Especialidad</td><td style="padding:8px;font-weight:600;">${specialty || 'No especificada'}</td></tr>
          ${description ? `<tr><td style="padding:8px;color:#64748b;">Descripción</td><td style="padding:8px;font-weight:600;">${description}</td></tr>` : ''}
          <tr><td style="padding:8px;color:#64748b;">Cliente</td><td style="padding:8px;font-weight:600;">${name || 'Anónimo'}</td></tr>
          <tr><td style="padding:8px;color:#64748b;">Email</td><td style="padding:8px;font-weight:600;">${clientEmail || 'No proporcionado'}</td></tr>
        </table>
        <a href="${siteUrl}/admin/" style="display:inline-block;padding:12px 28px;background:#D97706;color:white;text-decoration:none;border-radius:8px;font-weight:700;">Ir al panel admin</a>
      `),
    },
    ...
    new_specialty: {
      ...
      html: wrapEmail('Nueva Especialidad', '#8B5CF6', `
        <h2 style="color:#8B5CF6;margin:0 0 8px;">Nuevo especialista registrado</h2>
        <p style="color:#64748b;font-size:15px;">Se registró con una categoría que no existe en la plataforma.</p>
        <table style="width:100%;border-collapse:collapse;margin:16px 0;">
          <tr><td style="padding:8px;color:#64748b;">Especialidad sugerida</td><td style="padding:8px;font-weight:600;">${specialty || 'Otro'}</td></tr>
          <tr><td style="padding:8px;color:#64748b;">Especialista</td><td style="padding:8px;font-weight:600;">${name || ''}</td></tr>
          <tr><td style="padding:8px;color:#64748b;">Email</td><td style="padding:8px;font-weight:600;">${clientEmail || 'No proporcionado'}</td></tr>
        </table>
        <a href="${siteUrl}/admin/" style="display:inline-block;padding:12px 28px;background:#8B5CF6;color:white;text-decoration:none;border-radius:8px;font-weight:700;">Revisar en admin</a>
      `),
    },
